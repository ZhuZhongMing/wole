        ...
        <a-row>
          ...
          <a-col :xs="24" :sm="12">
            <a-form-item label="订单标题" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <j-dict-select-tag disabled type="list" v-decorator="['orderId', validatorRules.orderId]" :trigger-change="true" dictCode="mbp_order,order_title,id" placeholder="请选择订单编号" @change="changeOrder"/>
            </a-form-item>
          </a-col>
          <!--<a-col :xs="24" :sm="12">
    ...
    data() {
      return {
        ...
        url: {
          ...
          countOrderIfSet: '/system/mbpMainplanabstract/countOrderIfSet'
        }
      }
    },
    methods: {
     ...
     popupCallback(row){
      ...
      edit(record) {
        ...
        this.model = Object.assign({}, planRecord)
      ...
      request(formData) {
        getAction(this.url.countOrderIfSet, {orderId: formData.orderId}).then((res) => {
          if (res.success) {
            var count = res.result
            if (count > 0){
              this.$message.warning("该订单已指定过生产计划")
            } else {
     ...
     changeOrder(e) {
       if (e){
         ...
         getAction(this.url.queryListToPlan,params).then((res)=>{
           if(res.success){
             this.$nextTick(()=>{
               this.mbpMainplanTable.loading = true
               this.mbpMainplanTable.dataSource.splice(0)
               ...
               this.mbpMainplanTable.loading = false
             })
           }
         });
       }
